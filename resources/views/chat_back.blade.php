    ...
    <script>
      ...
      @foreach($users as $user)
        ...
        function gotRemoteStream{{$user}}(event){
          console.log("gotRemoteStream{{$user}}");
      ...
      conn.onmessage = function(str){
        ...
        //console.log("we got data: " + str.data);
        message = JSON.parse(str.data);
        console.log("NICK: " + message.nick);
        @foreach($users as $user)
          @if($user == $nick)
            @continue;
          @endif
          if (message.type === 'offer' && message.nick === '{{$user}}') {
              pc{{$user}}.setRemoteDescription(new SessionDescription(message));
              createAnswer{{$user}}();
            }
            else if (message.type === 'answer' && message.nick === '{{$user}}') {
              console.log("message.type === 'answer'", message.nick, '=== {{$user}}');
            //  pc{{$user}}.setRemoteDescription(new SessionDescription(message));
              pc{{$user}}.setRemoteDescription(new SessionDescription(message));
            }
            else if (message.type === 'candidate' && message.nick === '{{$user}}') {
